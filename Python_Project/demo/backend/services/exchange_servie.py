...
# ---------- Cache đơn giản để giảm gọi API ----------
_cache = {}

def _cache_set(key, value, ttl_seconds=300):
    """Lưu dữ liệu vào cache với thời hạn TTL."""
    _cache[key] = {
        "value": value,
...
def convert(amount: float, from_cur: str, to_cur: str) -> Dict:
    """
    Chuyển đổi tiền tệ theo công thức:
        converted_amount = round(amount * rate, 2)

    - Lấy tỷ giá thực tế (rate) từ API exchangerate.host
    - Kiểm tra đầu vào hợp lệ
    - Cache dữ liệu trong 30 giây
    """
